...
export async function loadLocalActivitiesConfig({
  ...
  if (
    filesByLang &&
    typeof filesByLang === 'object' &&
    !Array.isArray(filesByLang)
  ) {
    const defaultLanguage = study?.default_language || 'en';
    const selectedLang = lang || defaultLanguage;
    activitiesFile =
      filesByLang[selectedLang] ||
      filesByLang[defaultLanguage] ||
      filesByLang.en;
...
export async function loadActivitiesConfig({
  ...
  if (preferBackend && studyName) {
    const backendUrl = new URL(
      `${apiBaseUrl}/studies/${studyName}/activities-config`,
  ...
  if (requireBackend) {
    throw (
      lastBackendError ||
      new Error('Backend activities config is required but could not be loaded')
    );
  }
...
export function getTimelineConfig(
  key,
  configData = getCachedActivitiesConfig()
) {
  if (!configData) {
