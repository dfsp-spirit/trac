(function () {
  ...
  function hasSignificantStyleChange(oldStyle, newStyle) {
    ...
    // Compare the objects
    const keys = new Set([
      ...Object.keys(oldStyleObj),
      ...Object.keys(newStyleObj),
    ]);
  ...
  function setupActivityObserver(timeline) {
    if (debugMode)
      console.log(
        'AttributeTracker: Setting up observer for activity blocks...'
      );
    ...
    if (debugMode)
      console.log(
        'AttributeTracker: Now watching for activity blocks and their changes.'
      );
  ...
  function waitForElement() {
    if (debugMode)
      console.log('AttributeTracker: Waiting for timeline to be created...');
