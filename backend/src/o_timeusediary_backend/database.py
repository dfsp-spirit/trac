...
def upgrade_db_schema(revision: str = "head") -> None:
    alembic_cfg = _alembic_config()
    alembic_command.upgrade(alembic_cfg, revision)


def show_db_current_revision() -> None:
    alembic_cfg = _alembic_config()
...
def _hydrate_study_texts_from_config(
    ...
    for field_name in _STUDY_TEXT_FIELDS:
        if getattr(study, field_name) is None:
            ...
            if config_value is not None:
                ...
                updated = True
    # Handle description initialization from config (may be i18n map)
    try:
    ...
    if description_map:
        # Store the i18n map in the single `description` JSON column if DB
        # doesn't already have a description value.
        if not getattr(study, "description", None):
            study.description = description_map
            updated = True
...
def initialize_db_schema() -> None:
    logger.info("Applying DB schema migrations to head.")
    upgrade_db_schema("head")
